...
from utils import *
from logger import *
from train_mnb import *
from test_mnb import *
...
add_arg('--logPath', dest = 'log_path', default='/misc/vlgscratch4/BrunaGroup/sulem/chem/log/test5/')

# Optimization options
add_arg('--batchsize', dest='batch_size', help='mini-batch size',default=10)
add_arg('--maxepoch', dest='max_epoch', help='num epochs', default=100)
...
add_arg('--nfeatures', dest='nfeatures', help='feature maps', default=15)
...
def main():
    ...
    t_stats = data_stats(train_target)
    mean = t_stats[2]
    std = t_stats[3]
    
    for i in range (Nvalid):
        valid_target[i] = valid_set[i][2][args.task]
    v_stats = data_stats(valid_target)
    ...
    for epoch in range (args.max_epoch):
        ...
        loss, error = train_with_mnb(gnn, train_set, args.task, criterion,
    ...
    loss, error_ratio = test_with_mnb(gnn, valid_set, args.task, args.cuda, args.batch_size, mean, std, logger)
    loss_lg, error_ratio_lg = test_with_mnb(gnn_lg, valid_set, args.task, args.cuda, args.batch_size, mean, std, logger)
