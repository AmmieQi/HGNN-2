...
class GNN_simple(nn.Module):
    ...
    def __init__(self, task, n_features, n_layers, dim_input, J):
        ...
        self.layer0 = layers_mnb.layer_simple(self.featuremap_in, J+2)
        for i in range(n_layers):
            module = layers_mnb.layer_simple(self.featuremap_mi, J+2)
...
class GNN_lg(nn.Module):
    ...
    def __init__(self, task, n_features, n_layers, dim_input, J):
        ...
        self.layer0 = layers_mnb.layer_with_lg(self.featuremap_in, J+2)
        for i in range(n_layers):
            module = layers_mnb.layer_with_lg(self.featuremap_mi, J+2)
            self.add_module('layer{}'.format(i + 1), module)
