...
class layer_simple(nn.Module): 
    ...
    def forward(self, state, N_batch, mask):
        ...
        #zb1, _, _ = batching.batch_normalization(zb1,N_batch,mask)
        zb1, _, _ = batching.spatial_normalization(zb1,N_batch,mask)
        return (zb1, W)
...
class layer_last(nn.Module):
    ...
    def forward(self, state, N_batch, mask):
        ...
        x1 = graph_op(W,X)
        y1 = self.fc(x1)
        #y1, _, _ = batching.batch_normalization(y1,N_batch,mask)
        y1, _, _ = batching.spatial_normalization(y1,N_batch,mask)
        y = torch.sum(y1.squeeze(),dim=1)
        return y
...
class layer_with_lg(nn.Module): 
    ...
    def forward(self, state, N_batch, mask, E_batch, mask_lg):
        ...
        #zdb1, _ , _ = batching.batch_normalization(zdb1,E_batch,mask_lg)
        zdb1, _ , _ = batching.spatial_normalization(zdb1,E_batch,mask_lg)
